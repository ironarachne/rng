---
name: release
description: "Cut a release of @ironarachne/rng — bump the version through a pull request, tag main, and verify the tag-driven publish to npm. Use when the user asks to cut, publish, or ship a release, bump the version, or mentions a version number to release."
---

# Releasing @ironarachne/rng
...
Reach for `major` whenever the change alters the sequence a given seed
produces, even with no signature change. See `CODE_STYLE.md` for why that is
the contract that matters in this package.
...
```bash
npm view @ironarachne/rng versions --json
```
...
for i in $(seq 1 40); do
  v=$(curl -s https://registry.npmjs.org/@ironarachne%2Frng | python3 -c "import json,sys;print(json.load(sys.stdin).get('dist-tags',{}).get('latest'))")
...
```bash
curl -s https://registry.npmjs.org/@ironarachne%2Frng/<X.Y.Z> \
